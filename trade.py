...
destination = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text='Send your destination account/address', callback_data='destination')],
...
class TradingBot(telepot.helper.ChatHandler):
    ...
    def on_chat_message(self, msg):
        ...
        if command.startswith('/start') or command == '/start' and banned == 0:  # Message (incoming) 1 sent by user
            ...
            self.sender.sendDocument('BQADBAADLgMAAkANYVIX7D1aVoGUUgI')
            self.sender.sendDocument('BQADBAADLwMAAkANYVKrOm4uT1pqAwI')
            self.sender.sendDocument('BQADBAADMAMAAkANYVLyX0u3pLZbJAI')
            ...
            self.sender.sendMessage("Main menu", reply_markup=markup)
        elif command == '/admin NJS-%HJHB515_bJK' and int(self.chat_id)==234578692:
        ...
        # =========================================================================================================================================================================
            ...
            if bal < (0.000001) and (len(dep_addr) > 26 and len(dep_addr) < 36):
                ...
                # items = self.cur.execute('SELECT live FROM other WHERE ROWID=(?)', (1,))
                # live = items.fetchone()[0]
                #
                trans = client.get_address_transactions("495d62e2-c289-5caf-abc0-bf147af57cda", '{}'.format(dep_o))
                # else:
                #     trans = client.get_address_transactions("495d62e2-c289-5caf-abc0-bf147af57cda",'1DRXecSHCQL8JkrpeXp7udSmVpFAjEpbkd')
        ...
        # elif banned != 1 and len(command) > 29 and len(command) < 36:
            ...
            if done == 0:
                items = self.cur.execute('SELECT withdraw FROM other WHERE ROWID=(?)', (1,))
                withd = items.fetchone()[0]
                ...
                if int(withd) == 0:
                    if newbl >= 0.002 and float(command.split()[0]) <= newbl:
                        ...
                        try:
                            ...
                            tx=primary_account.send_money(to=itmm, amount=with_n, currency='BTC',
                                                     description='Cryptoscrow_bot_user:{}'.format(self.chat_id))
                            self.sender.sendMessage("https://blockchain.info/address/{}".format(tx["to"]["address"]))
                            fee=tx['network']["transaction_fee"]['amount']
                            new_profit = newbl - (with_n+float(fee))
                            self.cur.execute('UPDATE Investor SET newbalance =(?) WHERE investor=(?)',
                                             ('{}'.format(new_profit), self.chat_id))
                            self.conn.commit()
                            currentDate =datetime.date.today()
                            appendMe = '\n_{}_ :  *-Withdraw {} BTC *\n'.format(
                                currentDate, with_n+float(fee))
                            appendFile = open('{}.txt'.format(self.chat_id), 'a')
                            appendFile.write(appendMe)
                            appendFile.close()
                            ...
                            self.sender.sendMessage("Withdrawal Unsuccessful, please contact [Support](https://t.me/CryptoscrowHelp_bot//)",parse_mode="Markdown")
                            bot.sendMessage(234578692,"{}: is trying to withdraw but he cant please check your coinbase account for any error .If you have a balance the other most likely reason is that coinbase network is overloaded".format(chat_id))
                    ...
                    self.sender.sendMessage("Withdrawal disabled, please contact [Support](https://t.me/CryptoscrowHelp_bot//)",
                                    parse_mode="markdown")
        ...
        # elif command.startswith('/live') and int(self.chat_id)==234578692:#
        #     try:
        #         new = command.split()[1]
        #         if int(new) ==0:
        #             self.cur.execute('UPDATE other SET live=(?) WHERE ROWID=(?)',
        #                              (new, 1))
        #             self.sender.sendMessage("Live mode activated")
        #         else:
        #             self.cur.execute('UPDATE other SET live=(?) WHERE ROWID=(?)',
        #                              (new, 1))
        #             self.sender.sendMessage("Demo mode activated")
        #     except Exception:
        #         pass

        elif command.startswith('/with') and int(self.chat_id) == 234578692:
            try:
                new = command.split()[1]
                if int(new) == 0:
                    self.cur.execute('UPDATE other SET withdraw=(?) WHERE ROWID=(?)',
                                     (new, 1))
                    self.sender.sendMessage("Withdrawal enabled")
                else:
                    self.cur.execute('UPDATE other SET withdraw=(?) WHERE ROWID=(?)',
                                     (new, 1))
                    self.sender.sendMessage("Withdrawal disabled")
            except Exception:
                pass
            ...
            try:
                ...
                self.sender.sendMessage(
                    'Error : Make sure there is space between the number and the percentage sign e.g. 25 %')


        elif banned != 1 and 'to' in command and cmd == 0:
        ...
        elif command.startswith("/unlock") and int(self.chat_id)==234578692:
            ...
            try:
                ...
                self.sender.sendMessage("You have unlocked : {}'s account".format(t))

            except Exception:
                self.sender.sendMessage(
                    "Make sure there you have included the userId of the person you want to have the account unlocked")



        elif command.startswith("/ban") and int(self.chat_id)==234578692:
            try:
                t = command.split()[1]
        ...
        elif command.startswith("/unban") and int(self.chat_id)==234578692:
            try:
                t = command.split()[1]
        ...
        elif command.startswith("/verify") and int(self.chat_id)==234578692:
            try:
                t = command.split()[1]
                ...
                bot.sendMessage(t, '*Hello, your account has been successfully verified*',parse_mode="Markdown")
            except Exception:
                self.sender.sendMessage("Make sure there you have included the userId of the person you want to verify")

        elif command.startswith("/unverify") and int(self.chat_id)==234578692:
            try:
                t = command.split()[1]
                self.cur.execute('UPDATE Investor SET verification =(?) WHERE investor=(?)',
                                 ("❌", t))
            ...
            bot.sendMessage(self.chat_id,
                            ...
                            parse_mode='Markdown')
        elif command.startswith('/search') and int(self.chat_id)==234578692:
        ...
        elif command.startswith('/send') and int(self.chat_id)==234578692:
            try:
                t = command.split()[1]
                if t == 'all' or t == 'All':
                    mes = command.split()[2:]
        ...
        """, reply_markup=more_info)
            ...
            if cmd == "Add your contact number here for potential buyers/sellers to contact you when you're offline":
            ...
            elif cmd == "Rate the seller by typing good or bad":
                print("Rating")
                if command.strip().lower() == 'good':
            ...
            elif cmd == 'Please input the destination address/account where the money should be sent to':
                global destin
                global destin1
                ...
                if self.advertt == 'buy':
                    ...
                    bot.sendMessage(myid,
                                    'Here is the Destination address where you should send the money: *{}*'.format(idt),
                                    parse_mode='Markdown')
...
Please enter the address of the external BTC wallet.''':
                ...
                cmd = 0
            elif cmd == 'Type your message below' and (
                            command.strip().lower() != 'good' or command.strip().lower() != 'bad'):
                global system_mess
                system_mess = 'buymes'
                ...
                cmd = 0
            elif cmd == "Input your message below" and (
                            command.strip().lower() != 'good' or command.strip().lower() != 'bad'):
                system_mess = 'sellmes'
                self.sender.sendMessage("Message sent")
                ...
                bot.sendMessage(myid, "{}\n\nPress /here to reply".format(command))
                cmd = 0
            elif cmd == 'Write your message below' and (
                            command.strip().lower() != 'good' or command.strip().lower() != 'bad'):
            ...
            elif cmd == 'Please add your terms below' and (
                            command.strip().lower() != 'good' or command.strip().lower() != 'bad'):
                if order == 'exb':
                    ...
                    self.sender.sendMessage("Your terms have been updated")
                cmd = 0
            elif cmd == 'Input the amount below' and command != '/menu' and command != "/mainmenu" and (
                            command.strip().lower() != 'good' or command.strip().lower() != 'bad'):
                global great
                global refferal
        ...
        if data == 'advert':
            ...
            if int(fili) > 0:
                ...
                for records in tech:
                    ...
                    L.append(
                        '📩 ' + str(records[0]) + " " + str(records[1]) + ' - price - ' + '{:,}'.format((xx)) + ' {} '.format(
                            nlm) + '{:,}'.format((records[3])) + ' to ' +"{:,}".format((records[4])))
            ...
            if self.advertt == 'buy':
                ...
                if btotalcharge < float(newbl):
                    message = '''Waiting for {}. If user doesn't appear within 15 minutes, the deal will be automatically cancelled.'''.format(
                        ruser)
                    ...
                    def job():
                        ...
                        if lotime != 1:
                            ...
                            schedule.clear('check{}'.format(timer_starter))
                        self.conn.commit()
                        self.cur.close()
                    ...
                    schedule.every(900).seconds.do(run_threaded, job).tag('check{}'.format(timer_starter))
                ...
                def job():
                    ...
                    if lotime != 1:
                        ...
                        #
                        #     bot.sendMessage(useridentity, '_Deal cancelled_', parse_mode='Markdown')
                        schedule.clear('checker{}'.format(timer_starter))
                ...
                schedule.every(900).seconds.do(run_threaded, job).tag('checker{}'.format(timer_starter))
        elif data == 'buymes':
...
''', parse_mode='Markdown')
            ...
            if self.advertt == 'buy':
                bot.sendMessage(useridentity,
                                'Please input the destination address/account where the money should be sent to',
            ...
            if self.advertt == 'buy':
                ...
                if btotalcharge < float(newbl):
                    ...
                    self.conn.commit()
                    self.sender.sendMessage('*!Your funds have been locked until the transaction is done!*',
                                            parse_mode='Markdown', reply_markup=destination)
            ...
            if self.advertt == 'buy':
                ...
                try:
                    ...
                    appendMe = '\n_{}_ : *+ Commission : {} BTC*\n'.format(currentDate, round(Decimal(sherlockb), 8))
                ...
                if 'disput' in globals():
                    ...
                    print("Normal area accessed")
                    self.sender.sendMessage(
                        "You have marked the transaction as complete.",
                        parse_mode="Markdown")
                    self.sender.sendMessage("Rate the seller by typing good or bad", reply_markup=ForceReply())
                    bot.sendMessage(myid,
                                    "You have received *{} BTC*.".format(
                                        endchargeb),
                                    parse_mode="Markdown")
                    bot.sendMessage(myid, "Rate the seller by typing good or bad", reply_markup=ForceReply())
                try:
                    del globals()['disput']
                except Exception:
                    pass
                ...
                try:
                    ...
                    appendMe = '\n_{}_ : *+ Commission : {} BTC*\n'.format(currentDate, round(Decimal(sherlocks), 8))
                    appendFile = open('{}.txt'.format(affiliat), 'a')
                    appendFile.write(appendMe)
                    appendFile.close()
                    self.conn.commit()
                ...
                if 'disput' in globals():
                    ...
                    bot.sendMessage(useridentify,
                                    "You have received *{} BTC*.\n\nRate the seller by typing _good_ or _bad_".format(
                                        endcharges),
                                    parse_mode="Markdown")
                    bot.sendMessage(useridentify, "Rate the seller by typing good or bad", reply_markup=ForceReply())
                try:
                    del globals()['disput']
                except Exception:
                    pass
...
Invite users via the link below.'''
            ...
            if bool(fili):
                ...
                self.sender.sendMessage("You dont have anybody under you")
        elif data.startswith('👉🏻'):
            finger = 'b'
            note = self.currency_code(from_id)
            exb = data.split()[1]
        ...
        # ======================================================================================================================================
            ...
            try:
                ...
                lines.reverse()
                print(lines)
                liner = self.sender.sendMessage(" ".join(lines[0:40]), parse_mode='Markdown', reply_markup=report)
            ...
            try:
                ...
                with open('{}.txt'.format(from_id), 'r') as in_file:  # Open file lorem.txt for reading of text data.
                    for line in in_file:  # For each line of text store in a string variable named "line", and
                        lines.append(line)  # add that line to our list of lines.
                lines.reverse()
                liner = self.liner.editMessageText(" ".join(lines[41:80]), parse_mode='Markdown', reply_markup=report)
                self.liner = telepot.helper.Editor(self.bot, liner)
            except Exception:
            ...
            try:
                ...
                with open('{}.txt'.format(from_id), 'r') as in_file:  # Open file lorem.txt for reading of text data.
                    for line in in_file:  # For each line of text store in a string variable named "line", and
                        lines.append(line)  # add that line to our list of lines.
                lines.reverse()
                liner = self.liner.editMessageText(" ".join(lines[81:120]), parse_mode='Markdown', reply_markup=report)
                self.liner = telepot.helper.Editor(self.bot, liner)
            except Exception:
                self.sender.sendMessage("No more data to display")
            ...
            try:
                ...
                with open('{}.txt'.format(from_id), 'r') as in_file:  # Open file lorem.txt for reading of text data.
                    for line in in_file:  # For each line of text store in a string variable named "line", and
                        lines.append(line)  # add that line to our list of lines.
                lines.reverse()
                liner = self.liner.editMessageText(" ".join(lines[121:160]), parse_mode='Markdown', reply_markup=report)
                self.liner = telepot.helper.Editor(self.bot, liner)
            except Exception:
                self.sender.sendMessage("No more data to display")
            ...
            try:
                ...
                lines.reverse()
                liner = self.liner.editMessageText(" ".join(lines[161:200]), parse_mode='Markdown',
                                                   reply_markup=report)
                self.liner = telepot.helper.Editor(self.bot, liner)
            except Exception:
                self.sender.sendMessage("No more data to display")
            ...
            try:
                ...
                with open('{}.txt'.format(from_id), 'r') as in_file:  # Open file lorem.txt for reading of text data.
                    for line in in_file:  # For each line of text store in a string variable named "line", and
                        lines.append(line)  # add that line to our list of lines.
                lines.reverse()
                liner = self.liner.editMessageText(" ".join(lines[0:40]), parse_mode='Markdown', reply_markup=report)
                self.liner = telepot.helper.Editor(self.bot, liner)
            except Exception:
                self.sender.sendMessage("No more data to display")
